...
function MemoSessionApp() {
  ...
  const createSessionInBackend = async (sessionData: { subject: string; goal: string; tags: string[] }) => {
    try {
      const sessionPayload = {
        subject: sessionData.subject,
        goal: sessionData.goal,
        duration: 25, // 기본 25분으로 고정
        break_duration: 5, // 기본 5분 휴식
  ...
  const updateSessionInBackend = async (sessionId: string, updates: Partial<PomodoroSession>) => {
    try {
      ...
      if (res && res.ok) {
        console.log("Session updated in backend")
        await fetchSessionData()
        return true
      } else {
        console.error("Failed to update session in backend")
        return false
      }
    } catch (err) {
      console.error("Error updating session in backend:", err)
      return false
    }
  }
  ...
  // Session update function for timer modifications
  const handleUpdateSession = async (updates: Partial<PomodoroSession>) => {
    if (!currentSession) return

    const success = await updateSessionInBackend(currentSession.id, updates)
    if (success) {
      const updatedSession = { ...currentSession, ...updates }
      setCurrentSession(updatedSession)

      // Update timer if duration changed
      if (updates.duration && currentPhase === "focus") {
        setTimeLeft(updates.duration * 60)
      }
    }
  }
              ...
              {appMode === "session" && (
                <>
                  <button
                    ...
                    className={`rounded-full bg-gradient-to-r from-red-500 to-red-600 flex items-center justify-center text-white shadow-lg hover:scale-105 transition-all ${
                      isScrolled ? "w-8 h-8" : "w-10 h-10"
                    }`}
                  >
                    <Plus className={`${isScrolled ? "w-4 h-4" : "w-5 h-5"}`} />
        ...
        {showNewSessionModal && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-end z-50">
            <div className="w-full max-w-md mx-auto backdrop-blur-xl bg-white/10 border-t border-white/20 rounded-t-3xl p-6 max-h-[80vh] overflow-y-auto">
              <NewSessionForm onSubmit={handleCreateNewSession} onCancel={() => setShowNewSessionModal(false)} />
            </div>
          </div>
        )}
...
const NewSessionForm: React.FC<{
  onSubmit: (data: { subject: string; goal: string; tags: string[] }) => void
  onCancel: () => void
}> = ({ onSubmit, onCancel }) => {
  ...
  const addTag = () => {
    if (currentTag.trim() && !tags.includes(currentTag.trim())) {
      const formattedTag = currentTag.trim().startsWith("#") ? currentTag.trim() : `#${currentTag.trim()}`
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-medium text-white/70">새 세션 만들기</h3>
      ...
      <div className="flex gap-3">
        ...
        <button
          ...
          className="flex-1 py-3 rounded-xl bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-medium"
        >
          세션 생성
        </button>
      </div>
